...
interface_test_() ->
  ...
  [ ?_test("A ~ ToInt -> Int" = ok_prg(ToIntIface, "to_int"))
  , ?_test("Int" = ok_prg(
      ToIntIface ++
      "impl ToInt for Int {\n"
      "  to_int(i) = i\n"
  ...
  , ?_test("Int" = ok_prg(
      ...
      "  to_int(_) = 8\n"
      "}\n"
      "foo = to_int(@erlang:make_ref() : Ref)",
      "foo"
    ))
  , ?_test("Int" = ok_prg(
      ToIntIface ++
  ...
  , ?_test("Int" = ok_prg(
      ...
      "  to_int(r) = r.target\n"
      "}\n"
      "foo = to_int({ foo = \"hi\", bar = true, target = -3 })",
      "foo"
    ))
  , ?_test("Int" = ok_prg(
      ToIntIface ++
  ...
  , ?_test("(Int, Int)" = ok_prg(
      ...
      "foo = (to_int((1, true)), to_int((1, false, 2)))",
      "foo"
    ))
