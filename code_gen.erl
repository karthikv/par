...
run_ast(Ast, Mod) ->
  Env = lists:foldl(fun(Node, FoldEnv) ->
    case Node of
      ...
      {{struct_token, _}, {con_token, _, Name}, FieldTEs} ->
        FoldEnv#{Name => {{global_fn, list_to_atom(Name)}, length(FieldTEs)}};
      {{struct_token, _}, {gen_te, {con_token, _, Name}, _}, FieldTEs} ->
        FoldEnv#{Name => {{global_fn, list_to_atom(Name)}, length(FieldTEs)}};
      _ -> FoldEnv
    end
  end, #{}, Ast),
...
rep({global, {var, Line, Name}, Expr}, Env) ->
  case Expr of
    {{fn, _}, Args, _} ->
...
rep({{enum_token, _}, _, OptionTEs}, _) ->
  FnOptionTEs = lists:filter(fun({option, _, ArgsTE}) ->
    length(ArgsTE) > 0
  end, OptionTEs),

  lists:map(fun({option, {con_token, Line, Name}, ArgsTE}) ->
...
rep({{struct_token, _}, StructTE, FieldTEs}, Env) ->
  {Line, Name} = case StructTE of
    {con_token, L, N} -> {L, N};
    {gen_te, {con_token, L, N}, _} -> {L, N}
  end,
  Atom = list_to_atom(Name),
...
rep({field, Expr, {var, Line, Name}}, Env) ->
  Atom = list_to_atom(Name),
  call(maps, get, [{atom, Line, Atom}, rep(Expr, Env)], Line);

rep({app, {con_var, Line, Name}, Args}, #{'*in_pattern' := true}=Env) ->
...
rep({native, {atom, Line, Mod}, {var, _, Name}, Arity}, _) ->
  Atom = list_to_atom(Name),
  {'fun', Line,
...
rep({{Op, Line}, Left, Right}, Env) ->
  ...
  case Op of
    ...
    '|>' ->
      case Right of
        {app, Expr, Args} -> rep({app, Expr, [Left | Args]}, Env);
        _ -> rep({app, Right, [Left]}, Env)
      end;
    _ ->
      Atom = case Op of
        '!=' -> '/=';
...
arity({native, _, _, Arity}, _) -> Arity;
